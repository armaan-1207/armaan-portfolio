...
function PulseSphere() {
  ...
  useFrame((state) => {
    ...
    // Radial rings scale controlled to fade out naturally inside camera view bounds
    const maxScale = 2.3;
    if (ring1Ref.current && ring1Ref.current.material) {
      const s1 = (t * 0.75) % maxScale;
  ...
  // Generate random multi-colored cyber particles (green, cyan, violet)
  ...
  return (
    <group ref={groupRef}>
      ...
      <points ref={particlesRef}>
        ...
        <pointsMaterial size={0.04} color="#00d9ff" transparent opacity={0.7} sizeAttenuation />
      </points>
    </group>
  );
}
